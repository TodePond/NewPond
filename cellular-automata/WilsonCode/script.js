...
const setRules = (number) => {
	...
	state.rules.set(0, rules[0])
	state.rules.set(1, rules[1])
...
on.load(() => {
	...
	const drawSnapshot = (snapshot, y) => {
		...
		context.fillRect(...[(rightEdge), (y * CELL_SIZE), canvas.width / 4 - rightEdge, (CELL_SIZE)].map(n => round(n)))
	...
	show.tick = () => {
		...
		else present.rightVoid = state.rules.get(0)

		if (previous.leftVoid) present.leftVoid = state.rules.get(11)
		else present.leftVoid = state.rules.get(0)
